...
class GalleryCommentsController: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate {
    ...
    func getData() {
        ...
        var request = URLRequest(url: URL(string: Constants.METHOD_GALLERY_ITEM_INFO)!, cachePolicy: .reloadIgnoringLocalCacheData, timeoutInterval: 60)
        ...
        URLSession.shared.dataTask(with:request, completionHandler: {(data, response, error) in
            ...
            do {
                ...
                if (responseError == false) {
                    ...
                    self.item = Photo(Response: itemsArray[0] as AnyObject, owner: true)
    ...
    func remove(commentId: Int, index: Int) {
        ...
        request.httpMethod = "POST"
        let postString = "clientId=" + String(Constants.CLIENT_ID) + "&accountId=" + String(iApp.sharedInstance.getId()) + "&accessToken=" + iApp.sharedInstance.getAccessToken() + "&commentId=" + String(commentId) + "&itemType=" + String(Constants.ITEM_TYPE_GALLERY);
        request.httpBody = postString.data(using: .utf8)
    ...
    func send() {
        ...
        request.httpMethod = "POST"
        let postString = "clientId=" + String(Constants.CLIENT_ID) + "&accountId=" + String(iApp.sharedInstance.getId()) + "&accessToken=" + iApp.sharedInstance.getAccessToken() + "&itemId=" + String(self.itemId) + "&replyToUserId=" + String(self.replyToId) + "&commentText=" + self.commentText + "&itemType=" + String(Constants.ITEM_TYPE_GALLERY);
        request.httpBody = postString.data(using: .utf8)
    ...
    func showContentScreen() {
        ...
        if (self.item.getOwner().getAllowGalleryComments() == 1) {
