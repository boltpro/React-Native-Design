...
class ViewGalleryItemController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func showReportMenu() {
        ...
        let hateAction = UIAlertAction(title: NSLocalizedString("label_report_hate", comment: ""), style: .default) { action in
            ...
            self.report(abuseId: 1) // 1 = Hate Speech
        }
        ...
        let nudityAction = UIAlertAction(title: NSLocalizedString("label_report_nudity", comment: ""), style: .default) { action in
            ...
            self.report(abuseId: 2) // 2 = Nudity
        }
        ...
        let fakeAction = UIAlertAction(title: NSLocalizedString("label_report_piracy", comment: ""), style: .default) { action in
            ...
            self.report(abuseId: 3) // 3 = Piracy
        }
    ...
    func like() {
        ...
        request.httpMethod = "POST"
        let postString = "clientId=" + String(Constants.CLIENT_ID) + "&accountId=" + String(iApp.sharedInstance.getId()) + "&accessToken=" + iApp.sharedInstance.getAccessToken() + "&itemId=" + String(self.item.getId()) + "&itemType=" + String(Constants.ITEM_TYPE_GALLERY);
        request.httpBody = postString.data(using: .utf8)
    ...
    func remove() {
        ...
        var request = URLRequest(url: URL(string: Constants.METHOD_GALLERY_ITEM_REMOVE)!, cachePolicy: .reloadIgnoringLocalCacheData, timeoutInterval: 60)
        request.httpMethod = "POST"
    ...
    func report(abuseId: Int) {
        ...
        request.httpMethod = "POST"
        let postString = "clientId=" + String(Constants.CLIENT_ID) + "&accountId=" + String(iApp.sharedInstance.getId()) + "&accessToken=" + iApp.sharedInstance.getAccessToken() + "&itemId=" + String(itemId) + "&abuseId=" + String(abuseId) + "&itemType=" + String(Constants.REPORT_TYPE_GALLERY_ITEM);
