...
class LikesController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func getData() {
        ...
        var request = URLRequest(url: URL(string: Constants.METHOD_LIKES_GET)!, cachePolicy: .reloadIgnoringLocalCacheData, timeoutInterval: 60)
        request.httpMethod = "POST"
        let postString = "clientId=" + String(Constants.CLIENT_ID) + "&accountId=" + String(iApp.sharedInstance.getId()) + "&accessToken=" + iApp.sharedInstance.getAccessToken() + "&itemId=" + String(self.itemId) + "&profileId=" + String(self.profileId);
